...
func TestAccIssueAlertResource(t *testing.T) {
	...
	check := func(alert string) resource.TestCheckFunc {
		return resource.ComposeTestCheckFunc(
			...
			// Conditions
			resource.TestCheckResourceAttrWith(rn, "conditions", func(value string) error {
				expected := jsontypes.NewNormalizedValue(testAccIssueAlertConfigConditionsJson)
				given := jsontypes.NewNormalizedValue(value)
				match, _ := expected.StringSemanticEquals(context.Background(), given)

				if !match {
					return fmt.Errorf("expected %s, got %s", expected, given)
				}
...
var testAccIssueAlertConfigConditionsJson = `[
	{
...
`, teamName, projectName, alertName, testAccIssueAlertConfigConditionsJson)
}
